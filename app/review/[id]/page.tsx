...
export default function ReviewPage() {
  ...
  // Prevent any auto-scrolling behavior completely
  useEffect(() => {
    ...
    // Store original scroll position (should be 0,0 for new page loads)
    const originalScrollY = window.scrollY;
    
    // Override scrollIntoView globally during component mount
    const originalScrollIntoView = Element.prototype.scrollIntoView;
    Element.prototype.scrollIntoView = function() {
      // Do nothing - completely disable scrollIntoView
      return;
    };
    
    // Override window.scrollTo during initial load
    const originalScrollTo = window.scrollTo;
    window.scrollTo = function() {
      // Do nothing during initial load
      return;
    };
    
    // Restore original functions after component is fully mounted
    const cleanup = setTimeout(() => {
      Element.prototype.scrollIntoView = originalScrollIntoView;
      window.scrollTo = originalScrollTo;
      
      // Ensure we're at the top after restoring functions
      if (window.scrollY !== 0) {
        originalScrollTo.call(window, 0, 0);
      }
    }, 1000); // Longer timeout to ensure all child components are mounted
    
    return () => {
      clearTimeout(cleanup);
      // Restore functions on cleanup
      Element.prototype.scrollIntoView = originalScrollIntoView;
      window.scrollTo = originalScrollTo;
    };
  }, []);
  ...
  return (
    <MainLayout>
      {/* Container with explicit scroll management */}
